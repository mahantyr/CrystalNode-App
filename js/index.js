...
{
  var available=false,msgType='close'
  if($('#'+key))
    {
    available=true
   // console.log(true)
...
function openTab(id) {
  ...
  str3=$('#'+id+' .item-after').text().replace('ago','')
...
//console.log(str6)
  if($('#'+id).attr('data')=='open') {
   ...
   str4='<div class="card">'+
    '<div class="card-header"><b>'+str5+'</b></div>'+
    '<div class="card-content card-content-padding">'+str1+'</div>'+
    '<div class="card-footer"><b>Closed </b>'+str3+' ago</div>'+
    '</div>' 
  }
  $('#tab-3').prepend(str4)
  if(clicked==0) {
  ...
  if(notification[id]) {
    cordova.plugin.notification.local.cancel(notification[id])
  }
  if(alarmString[id]) {
...
function snoozeAlarm(key) {
  ...
  if(parseInt(ss)==NaN || ss.length==0) {
    ss=60
  } else {
    ss=parseInt(ss)
    var sec = new Date().getTime()+ss*1000
 try {
        var sound='file://pingpong.mp3'
        cordova.plugins.notification.local.schedule({
        id: Math.floor(1000000000*Math.random()),
        title: "We are reminding you about a message once again.",
        text: $('#'+key+' .item-title').text()+"-"+$('#'+key+' .item-subtitle').text(),
        at: new Date(sec),
        sound:sound,
        vibrate:true
    });
